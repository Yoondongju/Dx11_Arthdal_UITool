...
HRESULT CRenderer::Initialize()
{
	...
	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Blur_X"), 1600.f * 0.5f, 900.f * 0.5f, 1600.f, 900.f)))
		return E_FAIL;

	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Blur_Y"), 1600.f * 0.5f, 900.f * 0.5f, 1600.f, 900.f)))
		return E_FAIL;
...
HRESULT CRenderer::Draw()
{
	...
	if (FAILED(Render_UI()))		// 배치랑 디폴트랑 z가 0일경우 그려진느 순서에 영향을 받는데 배치는 부모가 될일이 없고 자식이 될일만 잇으니가 뒤에 그리자
		return E_FAIL;
...
HRESULT CRenderer::Render_ShadowObj()
{
	...
	ViewPortDesc.Height = (_float)900.f;
	ViewPortDesc.MinDepth = 0.f;
	ViewPortDesc.MaxDepth = 1.f;
