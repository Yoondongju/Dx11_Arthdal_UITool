                     ...
                     _In_ int       nCmdShow)
{
	...
	while (true)
	{
        ...
        if ( 1/*fTimeAcc >= (1.f / 60.f) */ )   // 60초에 한번씩 들어온다 
        {       
            _float fTime60 = pGameInstance->Compute_TimeDelta(TEXT("Timer_60"));  // 근데 60초인세상에서 델타타임을 계산해

            pMainApp->Update(fTime60);
            pMainApp->Render();


            pGameInstance->FinalUpdate_Engine();
