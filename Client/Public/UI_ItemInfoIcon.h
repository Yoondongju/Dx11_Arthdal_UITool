...
class CUI_ItemInfoIcon final : public CUI
{
public:
	//   0번: 아이템 깔리는 최하위 패널 
	//   1번: 손가락   2번 아이템슬롯  , 8 번  선 11번 휴지통    12번 나가기  , 13번 장착 or 사용


private:
	CUI_ItemInfoIcon(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CUI_ItemInfoIcon(const CUI_ItemInfoIcon& Prototype);
	virtual ~CUI_ItemInfoIcon() = default;
...
public:
	static CUI_ItemInfoIcon* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
