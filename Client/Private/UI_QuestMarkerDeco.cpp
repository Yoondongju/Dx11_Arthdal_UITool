#include "stdafx.h"
#include "..\Public\UI_QuestMarkerDeco.h"

#include "GameInstance.h"

#include "Level_MapTool.h"

_uint CUI_QuestMarkerDeco::m_iNextInstanceId = 0;

CUI_QuestMarkerDeco::CUI_QuestMarkerDeco(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUI{ pDevice, pContext }
{

}

CUI_QuestMarkerDeco::CUI_QuestMarkerDeco(const CUI_QuestMarkerDeco& Prototype)
	: CUI{ Prototype }
{
}

HRESULT CUI_QuestMarkerDeco::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CUI_QuestMarkerDeco::Initialize(void* pArg)
{
	if (nullptr == pArg)
	{
		UI_DESC			Desc{};
...
void CUI_QuestMarkerDeco::Priority_Update(_float fTimeDelta)
{
	for (auto pChildUI : m_childUI_List)
		pChildUI->Priority_Update(fTimeDelta);
}

void CUI_QuestMarkerDeco::Update(_float fTimeDelta)
{
...
void CUI_QuestMarkerDeco::Late_Update(_float fTimeDelta)
{
	/* 직교투영을 위한 월드행렬까지 셋팅하게 된다. */
	__super::Late_Update(fTimeDelta);

	if (false == m_isCompleteBatch)		// 이거 문제있네  배치가 완료된거여도 부모지정을 안햇으면 그려지는거잖아?
	{
...
HRESULT CUI_QuestMarkerDeco::Render()
{
...
void CUI_QuestMarkerDeco::Render_Text()
{
...
void CUI_QuestMarkerDeco::ChangeTexture(const _wstring& LstrChangeTexturePrototype)
{
...
void CUI_QuestMarkerDeco::MouseClick()
{
...
void CUI_QuestMarkerDeco::MouseOn()
{
	m_isUIEventType[UI_EVENT_TYPE::MOUSE_OUT] = false;
	m_isUIEventType[UI_EVENT_TYPE::MOUSE_ON] = true;
}

void CUI_QuestMarkerDeco::MouseOut()
{
	m_isUIEventType[UI_EVENT_TYPE::MOUSE_ON] = false;
	m_isUIEventType[UI_EVENT_TYPE::MOUSE_OUT] = true;
}


void CUI_QuestMarkerDeco::ExcuteMouseClick(_float fTimeDelta)
{
	if (m_vTrigerTargetStartUV.x != 0.f &&
		m_vTrigerTargetStartUV.y != 0.f &&
		m_vTrigerTargetEndUV.x != 1.f &&
		m_vTrigerTargetEndUV.y != 1.f)
	{
		if (false == m_isCompleteBatch)		// 배칭UI가 아닐경우
		{
			if (false == m_isActiveTrigerUV)
				m_isActiveTrigerUV = true;
			else
				m_isActiveTrigerUV = false;
		}
		else
		{
			swap(m_vCurStartUV, m_vTrigerTargetStartUV);
			swap(m_vCurEndUV, m_vTrigerTargetEndUV);
		}
	}
...
void CUI_QuestMarkerDeco::ExcuteMouseOn(_float fTimeDelta)
{
	
}

void CUI_QuestMarkerDeco::ExcuteMouseOut(_float fTimeDelta)
{

}


HRESULT CUI_QuestMarkerDeco::Ready_Components(void* pArg)
{
	/* FOR.Com_Shader */
...
CUI_QuestMarkerDeco* CUI_QuestMarkerDeco::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CUI_QuestMarkerDeco* pInstance = new CUI_QuestMarkerDeco(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CUI_QuestMarkerDeco"));
...
CGameObject* CUI_QuestMarkerDeco::Clone(void* pArg)
{
	CUI_QuestMarkerDeco* pInstance = new CUI_QuestMarkerDeco(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CUI_QuestMarkerDeco"));
		Safe_Release(pInstance);
	}
